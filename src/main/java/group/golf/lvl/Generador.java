...
import group.golf.juego.Entorno;
import group.golf.juego.JuegoMain;
...
public class Generador {
	...
	try {
		fondo = ImageIO.read(new File("src/main/java/group/golf/img/fondo.png"));
	} catch (IOException e) {
		e.printStackTrace();
	}}
		...
		int x=0, y=0, f=0, cont; 
			...
			for (int u = 0 ; u<cant ; u++) {
				...
				while (!esValido && cont<5) {
					x = JuegoMain.generarRandom(24 , entorno.ancho()-24 , 1);
					y = JuegoMain.generarRandom(58 , entorno.alto()-24 , 1);
	...
	private void dibujarElementos(Entorno entorno, String nombre, List<Integer[]> lista) {
		Image imagen = null;
		try {
			imagen = ImageIO.read(new File("src/main/java/group/golf/img/" + nombre + ".png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private void generarTroncos(Entorno entorno) {
		int cant = JuegoMain.generarRandom(8, 16, 1);
	...
	private void dibujarAgujero(Entorno entorno) {
		Image aguj = null;
		try {
			aguj = ImageIO.read(new File("src/main/java/group/golf/img/agujero.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private void generarObstaculo(String tipo, Entorno entorno) {
		...
		int alto = JuegoMain.generarRandom(4,8,1) , ancho = JuegoMain.generarRandom(4,8,1);		
		...
		if (isEmpty()) {
			offsetX = JuegoMain.generarRandom( (16*(int)(ancho/2)+16) , (entorno.ancho()-(16*ancho))-16 , 1);
			offsetY = JuegoMain.generarRandom( (16*(int)(alto/2)+48) , (entorno.alto()-(16*alto))-16 , 1);
	...
	public void generarLvl(Entorno entorno) {
		...
		for (int i = 0 ; i<this.cantObstaculos ; i++ ) {
			int index = JuegoMain.generarRandom(0,tipos.length-1,1);
	...
	public void crearLvl(Entorno entorno, JuegoMain juego){
	...
	File file = new File("src/main/java/group/golf/lvl/lvl" + juego.getNivel() + ".png");
	...
	try {
		writeText = new PrintWriter(new File("src/main/java/group/golf/lvl/"+archivo), "UTF-8");
