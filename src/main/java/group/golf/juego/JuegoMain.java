...
import group.golf.lvl.Generador;
...
public class JuegoMain extends InterfaceJuego
{
	...
	private boolean existeNivel() {
		File img = new File("src/main/java/group/golf/lvl/lvl" + nivelActual + ".png");
		File data = new File("src/main/java/group/golf/lvl/lvl" + nivelActual + ".txt");
	...
	JuegoMain()
	{
		...
		this.entorno = new Entorno(this, "Golf (Entorno)", 800, 640);

		// Inicializar lo que haga falta para el juego
		try {
			menuImg = ImageIO.read(new File("src/main/java/group/golf/img/menu.png"));
			}
		catch (IOException e){
			e.printStackTrace();
		}		
